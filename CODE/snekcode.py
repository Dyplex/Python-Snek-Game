...
background_song = pygame.mixer.Sound('sounds/background_noise.mp3')
eat_sound = pygame.mixer.Sound('sounds/munch.mp3')
gaemover_sound = pygame.mixer.Sound('sounds/gaemover.mp3')
testsound = pygame.mixer.Sound('assets/cutscene/cutscene_snd.mp3')
...
def restart_program():#YEA SO UH FIX THIS LATER s
    python_executable = sys.executable  # Get the path to the Python executable
    script_file = __file__  # Get the path to the current script

    subprocess.Popen([python_executable, script_file])
    fereastra.destroy() 

    '''
    global sarpe, mancare, puncte, direction, panel1, puncte, previous_puncte, previous_live
    #canvas.delete(ALL)
    canvas.delete(sarpe, mancare)
    sarpe = Sarpe() 
    mancare = Mancare()
    puncte = 0
    previous_puncte = 15
...
#fereastra.bind('<r>', restart_game)
fereastra.bind('<r>', lambda event: restart_program())
